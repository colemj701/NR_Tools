import os, re, datetime, arcpy, logging
...
FeatureLayerId = arcpy.GetParameterAsText(0)

#What are your ArcGIS Enterprise/ArcGIS Online credentials? This is case sensitive.
PortalUserName = 'matthew.cole.701'
PortalPassword = '#Darwins7Liter0Box1'
PortalUrl = 'https://amecei.maps.arcgis.com/'

#Where do you want your attachments stored?
SaveAttachmentsTo = arcpy.GetParameterAsText(1)
SaveLogsTo = os.path.join(SaveAttachmentsTo, "Download_Log")
            ...
            if len(currentObjectIdAttachments) > 0:
                ...
                for k in range(len(currentObjectIdAttachments)):
                    ...
                    if AttachmentStorage == 'IndividualFolder':
                        ...
                        newAttachmentPath = os.path.join(currentFolder, fileName)
                        newAttachmentPath = newAttachmentPath.replace("\\","/")
                        if not os.path.isfile(newAttachmentPath):
                            ...
                            currentAttachmentPath = str(featureLayer.attachments.download(oid=currentObjectId, attachment_id=attachmentId, save_path=currentFolder))
                            currentAttachmentPath = currentAttachmentPath.replace("\\\\","/")[2:-2]
                        ...
                        newAttachmentPath = os.path.join(featureLayerFolder, fileName)
                        if not os.path.isfile(newAttachmentPath):
                            logger.info('The size of the current attachment being downloaded is {}MB'.format((attachmentSize/1000000)))
